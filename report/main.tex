...
% FIX: removed left header text; only page number at top-right as required
...
% ─── Hyperref ───────────────────────────────────────────────────────────────
% Link colours set to black for formal hackathon submission
\hypersetup{
  colorlinks=true,
  linkcolor=black,
  urlcolor=black,
...
% ─── Project metadata ───────────────────────────────────────────────────────
\newcommand{\hackathonname}{Bano Qabil $\times$ Alibaba Cloud AI Hackathon 2026}
\newcommand{\hackathonorg}{Bano Qabil Pakistan}
\newcommand{\cloudpartner}{Alibaba Cloud}
  ...
  \IfFileExists{#2}{%
    ...
    \fcolorbox{gray}{gray!10}{%
      \parbox[c][0.22\textheight][c]{0.86\linewidth}{%
        \centering\small\textit{Figure placeholder}\\[4pt]\texttt{\detokenize{#2}}%
      }%
    }%
  }%
}
...
\newcommand{\appscreenshotLandscape}[2]{%
  \begin{figure}[H]
    \centering
    \safeincludegraphics[width=0.88\textwidth,keepaspectratio]{#1}
    \caption{#2}
  \end{figure}%
}
\newcommand{\appscreenshotPortrait}[2]{%
  \begin{figure}[H]
    \centering
    \safeincludegraphics[width=0.42\textwidth,keepaspectratio]{#1}
    \caption{#2}
  \end{figure}%
}
...
\newcommand{\storyboardgrid}[6]{%
  \begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.47\textwidth}
      \centering
      \safeincludegraphics[width=\linewidth]{#2}
    ...
    \begin{subfigure}[t]{0.47\textwidth}
      \centering
      \safeincludegraphics[width=\linewidth]{#3}
      \caption{Frame 3: Home / Classrooms}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.47\textwidth}
      \centering
      \safeincludegraphics[width=\linewidth]{#4}
    ...
    \begin{subfigure}[t]{0.47\textwidth}
      \centering
      \safeincludegraphics[width=\linewidth]{#5}
      \caption{Frame 5: Live AR Camera}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.47\textwidth}
      \centering
      \safeincludegraphics[width=\linewidth]{#6}
...
% FIX: force full justification throughout the document
...
\begin{titlepage}
  ...
  \begin{center}
    \vspace*{1.5cm}
    {\large \hackathonname}\\[0.3cm]
    {\large Hackathon Project Submission}\\[0.2cm]
    {\small Organised by \hackathonorg \quad|\quad Powered by \cloudpartner}\\[1.2cm]
    ...
    {\Large Muhammad Abdullah}\\[0.25cm]
    {\normalsize bscs23070@itu.edu.pk}\\[0.6cm]
    {\small Unity AR \textbullet\ Designer Studio \textbullet\ Backend \textbullet\ AI}
...
LucidLab is submitted to the Bano Qabil $\times$ Alibaba Cloud AI Hackathon 2026 as an
AI-powered AR education platform. The project addresses a practical gap in science
education where students often learn theory without immersive experimentation due to lab
resource limits, safety concerns, and limited guided feedback outside physical labs. The
proposed solution,
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item Firebase (authentication and Firestore-backed classroom data workflows).
...
template (rough pencil/ink, single phone silhouette) for a consistent visual style.
...
LucidLab was designed, implemented, and deployed by \textbf{Muhammad Abdullah}.
The author contact is \texttt{bscs23070@itu.edu.pk}. The work spans the full platform:

\begin{itemize}[leftmargin=1.4cm]
  \item Unity AR mobile app --- runtime, WebView shell, Vuforia experiments,
        and AI (Vapi/Gemini) integration.
  \item Designer Studio --- VPL node editor, scene logic system, EditorRenderer
        (Unity WebGL preview), React frontend, UI/UX, and component library.
  \item Backend --- Firebase Auth, Firestore, Gemini AI logic-generation
        endpoint, Supabase storage, asset pipeline, and build optimisation.
\end{itemize}
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \begin{itemize}
    \item Given hackathon timelines, advanced fluid mechanics and high-fidelity simulation
...
LucidLab AR Science Learning Platform, developed for the Bano Qabil $\times$ Alibaba Cloud
AI Hackathon 2026, presents a practical, evidence-backed approach for
